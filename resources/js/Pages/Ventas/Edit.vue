...
const form = useForm({
    ...
    fecha: formatDate(props.venta.fecha),
    tipo: props.venta.tipo.toString(),
    detalles: props.venta.detalle_ventas?.map(det => ({
        id: det.id,
        receta_id: det.receta_id,
        nombre: det.receta?.nombre || 'Producto',
        descripcion: det.receta?.descripcion,
        tiempo_preparacion: det.receta?.tiempo_preparacion,
        insumos_count: det.receta?.insumos?.length || 0,
...
const submit = () => {
    form.put(`/ventas/${props.venta.id}`);
};
</script>

<template>
    <MainLayout title="Editar Venta">
